...
- This workspace has an app-level quality gate for its own Python (`main.py`, `src/**`), added on top of Caspian — the framework itself ships no test runner. One command, `npm run check` (which calls `uv run python settings/check.py`), runs `pyrefly` (types), `ruff` (lint), and `pytest` (tests) in a single pass and prints each problem as `path:line:col [tool:code] message`, exiting non-zero on failure. Run it after editing app-owned Python and before treating a change as done; fix the reported locations. `npm run check` only reports — auto-fix ruff findings with `npm run check:fix` (runs `ruff check . --fix` then re-runs the gate). One deliberate exception: `F401` (unused imports) is marked `unfixable` in `pyproject.toml`, so `--fix` never deletes imports — Caspian single-file components import children and use them only as `<x-*>` tags (`from .Dialog import DialogContent` → `<x-dialog-content>`), which ruff can't see; `settings/check.py` suppresses those F401s and fails only on genuinely dead imports, which you remove by hand after confirming they are not used as an `<x-*>` tag. See `tests/README.md`. Tests live in `tests/`; tooling and config live in `pyproject.toml` (`[dependency-groups] dev`, `[tool.pyrefly]`, `[tool.ruff]`, `[tool.pytest.ini_options]`); install them with `uv sync --group dev`. Note `[tool.pyrefly.errors]` suppresses `bad-return` and `bad-assignment`, so those specific type-error kinds are not flagged. Because this is project-specific, keep it documented here and in `.github/copilot-instructions.md`, not in the packaged docs.
