...
- `npm run check` only reports. Auto-fix ruff findings with `npm run check:fix` (`ruff check . --fix-only` then the gate). It uses `--fix-only` so the fix step always exits `0` and the gate re-runs; plain `--fix` would short-circuit the `&&` whenever unfixable findings remain. pyrefly and pytest failures are never auto-fixed.
- `F401` (unused imports) is `unfixable` in `[tool.ruff.lint]`, so `--fix` never deletes imports. Single-file components import children used only as `<x-*>` tags in `html(...)`/`render_html(...)` templates (`from .Dialog import DialogContent` → `<x-dialog-content>`), which ruff cannot see; Caspian resolves those tags from module globals at render time, so deleting the import breaks rendering. `settings/check.py` suppresses the `F401`s whose symbol is used as an `x-{camel_to_kebab(name)}` tag in the same file and fails only on genuinely dead imports. When the gate reports an `F401`, remove it by hand only after confirming it is not an `<x-*>` tag; never re-enable `F401` autofix. See `node_modules/caspian-utils/dist/docs/testing.md`.
